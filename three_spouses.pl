...
% ----
% +  Вхідний параметр (має бути зв''язаний перед викликом)
% -  Вихідний параметр (буде зв''язаний предикатом)
% ?  Може бути вхідним або вихідним
% -- Мультипризначений предикат (генерує кілька розв''язків при backtracking)
%
% Аналіз мультипризначенності:
% ----
%
% Детерміновані предикати (генерують 1 розв''язок):
% • people/1, initial_state/1, goal_state/1
% • canon_state/2, safe_bank/1, safe_state/1
% • solve_bfs/1, solve_bfs_dynamic/1, solve_dfs/1
% • print_solution/1, compare_methods/0
%
% Мультипризначені предикати (генерують багато розв''язків):
% • husband/1 - генерує: h1; h2; h3
% • wife/1 - генерує: w1; w2; w3
% • couple/2 - генерує: (h1,w1); (h2,w2); (h3,w3)
% • passengers/2 - генерує всі комбінації 1-2 пасажирів
% • neighbor/2 - генерує всі можливі безпечні переходи
%
% Приклад мультипризначенності:
% ?- passengers([h1, h2, w1], X).
% X = [h1] ;
% X = [h1, h2] ;
% X = [h1, w1] ;
% X = [h2] ;
...
% МУЛЬТИПРИЗНАЧЕНИЙ при повністю незв''язаних параметрах
% Детермінований при зв''язаному одному параметрі
couple(h1, w1).
couple(h2, w2).
couple(h3, w3).

% -- husband(?Person)
% Предикат для ідентифікації чоловіків
% МУЛЬТИПРИЗНАЧЕНИЙ: генерує всіх чоловіків при backtracking
husband(h1). 
husband(h2). 
husband(h3).

% -- wife(?Person)
% Предикат для ідентифікації дружин
% МУЛЬТИПРИЗНАЧЕНИЙ: генерує всіх дружин при backtracking
wife(w1). 
wife(w2). 
wife(w3).
...
% Детермінований предикат (успіх/невдача)
safe_bank(Bank) :-
...
% -- passengers(+Bank, -Passengers)
% Генерація можливих комбінацій пасажирів для переправлення
% МУЛЬТИПРИЗНАЧЕНИЙ: генерує різні варіанти при backtracking
%
% З берега [h1, h2, w1] може згенерувати:
% [h1], [h2], [w1], [h1,h2], [h1,w1], [h2,w1]
% (всього 6 варіантів)
...
% Генерація наступних допустимих станів
% МУЛЬТИПРИЗНАЧЕНИЙ: генерує всі можливі безпечні переходи
%
% З початкового стану state(left,[h1,h2,h3,w1,w2,w3],[])
% генерує близько 5 безпечних переходів при backtracking

% Переправлення з лівого берега на правий
neighbor(state(left, L, R), Next) :-
    passengers(L, Ps),       % МУЛЬТИПРИЗНАЧЕНИЙ вибір пасажирів
    move_list(Ps, L, L1),
    add_list(Ps, R, R1),
...
neighbor(state(right, L, R), Next) :-
    passengers(R, Ps),        % МУЛЬТИПРИЗНАЧЕНИЙ вибір пасажирів
    move_list(Ps, R, R1),
    add_list(Ps, L, L1),
...
% + solve_bfs(-Path)
% Знаходить оптимальний шлях методом BFS
% Детермінований предикат
solve_bfs(Path) :-
    initial_state(I),
    bfs_list([[I]], [I], Rev),
...
% + bfs_list(+Queue, +Visited, -Solution)
% Рекурсивний BFS з чергою шляхів
% Детермінований предикат
...
% ---
% Метод 2: Пошук у ширину (BFS) з динамічними предикатами
% ---
...
% Детермінований предикат з побічним ефектом
...
% Знаходить оптимальний шлях методом BFS з динамічною базою
% Детермінований предикат
solve_bfs_dynamic(Path) :-
    init_visited,
    initial_state(I),
...
% + bfs_dyn(+Queue, -Solution)
% Рекурсивний BFS з динамічною базою
% Детермінований предикат
...
% + solve_dfs(-Path)
% Знаходить шлях методом DFS (може бути неоптимальним)
% Детермінований предикат
solve_dfs(Path) :-
    initial_state(I),
...
% + dfs_limited(+State, +Path, +Depth, -Solution)
% Рекурсивний DFS з обмеженням глибини
% Детермінований предикат
...
dfs_limited(S, Path, Depth, Sol) :-
    Depth > 0,
    neighbor(S, N),       % МУЛЬТИПРИЗНАЧЕНИЙ вибір наступного стану
    \+ member(N, Path),
    Depth1 is Depth - 1,
...
% + compare_methods
% Виконує порівняльний аналіз всіх трьох методів
% Детермінований предикат
compare_methods :-
    nl,
    write('---'), nl,
...
%
% 8) Демонстрація мультипризначенності:
%    ?- husband(X).              % X = h1 ; X = h2 ; X = h3
%    ?- wife(X).                 % X = w1 ; X = w2 ; X = w3
%    ?- couple(X, Y).            % Генерує всі 3 пари
%    ?- passengers([h1,h2], X).  % Генерує [h1], [h2], [h1,h2]
%
% ---